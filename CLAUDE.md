...
```
project-root/
├── src/pm_pedia_langextract/  # メインパッケージ
...
2. **実装例の確認**
   - @src/pm_pedia_langextract/core/example.py で適切な型ヒント、docstring、エラーハンドリングを確認
   - @src/pm_pedia_langextract/types.py で型定義のパターンを確認

3. **ユーティリティ実装**
   - @src/pm_pedia_langextract/utils/helpers.py で関数の構造、エラー処理を確認
